...
class Utils extends BaseUtils
{
    ...
    static function generateBladeView($subject, $data = [])
    {
        ...
        $component = new class($subject) extends \GSManager\View\Component
        {
            protected $template;
    ...
    static function applyMiddleware(\GSManager\Http\Request $request, $middleware = [])
    {
        $response = (new \GSManager\Pipeline\Pipeline(app()))
            ->send($request)
            ->through($middleware)
            ->then(function() {
                return new \GSManager\Http\Response();
            });

        if ($response instanceof \GSManager\Http\RedirectResponse) {
            abort($response);
        }
