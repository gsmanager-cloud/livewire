...
/** @mixin \GSManager\Testing\TestResponse */
...
class Testable
{
     ...
     * @return void
     */
    static function actingAs(\GSManager\Contracts\Auth\Authenticatable $user, $driver = null)
    {
    ...
    {
        if ($value instanceof \GSManager\Http\UploadedFile) {
            return $this->upload($name, [$value]);
        } elseif (is_array($value) && isset($value[0]) && $value[0] instanceof \GSManager\Http\UploadedFile) {
    ...
    function upload($name, $files, $isMultiple = false)
    {
        ...
        } catch (\GSManager\Validation\ValidationException $e) {
